...
export function useYellowSession(): YellowSession {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log(
        "[Yellow] Client initialized with session key:",
  ...
  const refreshBalance = useCallback(async () => {
    ...
    if (DEFAULT_ADJUDICATOR === "0x0000000000000000000000000000000000000000") {
      const mockBalance =
        localStorage.getItem(`edge60_mock_balance_${address}`) || "0.0";
  ...
  const deposit = useCallback(
    async (amount: string) => {
      ...
      try {
        ...
        if (
          DEFAULT_ADJUDICATOR === "0x0000000000000000000000000000000000000000"
        ) {
          const currentMock = parseFloat(
            localStorage.getItem(`edge60_mock_balance_${address}`) || "0.0"
          );
